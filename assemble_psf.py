...
def assemble_psf(config):
    ...
    totalweight_fitted = np.sum(psf_fitted_smoothed) - psf_fitted_smoothed[resolution//2, resolution//2]
    print('Total weight fitted', totalweight_fitted)

    psf_new =  psf_orig * (1. - totalweight_fitted) + psf_fitted_smoothed
    psf_new[resolution//2, resolution//2] = 1. - (np.sum(psf_new) - psf_new[resolution//2, resolution//2])
